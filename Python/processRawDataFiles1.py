...
# a different version                                  blue     apricot        
# mpl.rcParams['axes.prop_cycle'] = mpl.cycler(color=["#005a8a", "#e6a176", "#8a034f", "#cacaca", "#005900"])
...
if (exists(folder + "logFile.csv")) and ('Temperature(room)' in dfLogFile.columns):
    ...
    if 'TargetT(box)' in dfLogFile.columns:
        ...
        labelSP = str("SP: ") + str(dfLogFile['TargetT(box)'].iat[0]) + " °C"
...
if 'pressureA' in dfLogFile.columns:
    ...
    plt.plot(x, y)
    if(measurement_date < datetime(2022, 12, 20) ):
        # The baratron was exchanged
        unit = "Torr)"
    else:
        unit = "mbar)"
    plt.ylabel("A pressure (" + unit)
    ax10b = ax10.twinx()
